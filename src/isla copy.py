    ...
    for i in range(len(mapa)):
        for j in range(len(mapa[0])):
            if mapa[i][j] != CELDA_TESORO:
                ...
                #este if evita que aparezca una trampa o pista en la posicion inicial del jugador
                if mapa[2][2] == CELDA_TRAMPA or mapa[2][2] == PISTA_ENCONTRADA:
                    mapa[2][2] = CELDA_VACIA
...
def imprimir_mapa_oculto(mapa: list,posicion_jugador:tuple):
    ...
    print("   1 2 3 4 5")
    print("   ---------")
    #Inicialmente solo estaba el print de dentro del for, todo lo demas añadido es como para hacer un tablero con las posiciones de las filas, colunas
    contafila = 0
    for fila in mapa:
        print(f"{contafila + 1} |"+ " ".join([simbolo_celda(celda) for celda in fila]) + "|")
        contafila += 1
    print("   ---------")
...
def imprimir_mapa(mapa: list):
    ...
    """
    #creo que esto cuenta como mejora mia, si pones s se muestra todo y con el numero de filas y columnas
    print("     1    2    3    4    5")
    print("   ------------------------")

    contafila = 0
    for fila in mapa:
        print(f"{contafila + 1} |"+ f"{fila}" + "|")
        contafila += 1
    print("   ------------------------")
...
def muestra_resultado_del_movimiento(resultado: int, nueva_posicion: tuple, mapa: list):
    ...
    if resultado == MOVIMIENTO_INVALIDO:
        ...
        muestra_estado_mapa(mapa,nueva_posicion)
...
def muestra_estado_mapa(mapa, posicion_jugador:tuple):
    """Muestra el mapa y la posición del jugador."""
    
    imprimir_mapa_oculto(mapa,posicion_jugador)
    #fila esto y columna esto añaden 1 a ambas posiciones para que se hagan un print
    fila_esto = posicion_jugador[0] + 1
    columna_esto = posicion_jugador[1] + 1
    print(f"Tu posición es ({fila_esto},{columna_esto})")
